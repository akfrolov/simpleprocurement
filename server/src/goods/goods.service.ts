...
function saveImage(
  ...
  fs.writeFile(`./public/images/${goodId}/${data.title}`, data.src.split(",")[1],
    { encoding: "base64" },
    (error) => {
      if (error) {
        console.log("Error");
...
export class GoodsService {
  ...
  async create(createGoodDto: CreateGoodDto) {
    ...
    if (images) {
      const imageLinks = images.map(
        (data) => saveImage(data, doc.id));
      doc = await this.goodModel.findByIdAndUpdate(
        doc.id, { images: imageLinks }
      ).exec();
    }
    return doc.toObject();
  }
  ...
  async update(id: string, updateGoodDto: UpdateGoodDto) {
    if (updateGoodDto.images && updateGoodDto.images.length > 0) {
      updateGoodDto.images = updateGoodDto.images.map(
        (data) => saveImage(data, id));
    }
  ...
  async deleteMany(ids: string[]) {
    ids.map(async (id) => {
      const good = await this.goodModel.findByIdAndDelete(
        id
        // { _id: { $in: ids } }
      ).exec();
      deleteImages(good);
